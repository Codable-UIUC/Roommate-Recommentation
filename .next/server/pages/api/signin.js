...
}).use(_middleware_requireAuth__WEBPACK_IMPORTED_MODULE_3__/* ["default"] */ .Z).post((req, res)=>{
    const { email , password  } = JSON.parse(req.body);
    _library_logger__WEBPACK_IMPORTED_MODULE_4__/* ["default"].info */ .ZP.info("api/signin  methodd : post");
    (0,_library_mongodb__WEBPACK_IMPORTED_MODULE_2__/* .verifyPassword */ .Gv)(email, password).then((result)=>{
        if (result == "wrong") return res.json({
            data: "wrong password"
        });
        if (result == "no matching id") return res.json({
            data: "no matching email"
        });
        const id = result;
...
var __webpack_exports__ = __webpack_require__.X(0, [686,441], () => (__webpack_exec__(5227)));
